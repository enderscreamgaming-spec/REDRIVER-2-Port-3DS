...
#ifdef __3DS__
#define GAME_HIRES_FONT_SCALE 0.33f
#define GAME_HIRES_FONT_Y_OFFSET 13.0f
#define GAME_HIRES_FONT_SHADOW_OFFSET 1.0f
#else
#define GAME_HIRES_FONT_SCALE 0.275f
#define GAME_HIRES_FONT_Y_OFFSET 14.0f
#define GAME_HIRES_FONT_SHADOW_OFFSET 0.5f
#endif
...
void GetHiresBakedQuad(int char_index, float* xpos, float* ypos, FONT_QUAD* q)
{
	...
	float scale = GAME_HIRES_FONT_SCALE;
...
int PrintStringHires(char* string, int x, int y)
{
	...
	while ((chr = *string++) != 0)
	{
		...
		if (showMap == 0)
		{
			...
			setXYWH(shadowFT4, q.x0 + GAME_HIRES_FONT_SHADOW_OFFSET, q.y0 + GAME_HIRES_FONT_SHADOW_OFFSET, q.x1, q.y1);
		...
		else
		{
			...
			setXYWH(fontFT4, q.x0 + GAME_HIRES_FONT_SHADOW_OFFSET, q.y0 + GAME_HIRES_FONT_SHADOW_OFFSET, q.x1, q.y1);
			DrawPrim(fontFT4);
