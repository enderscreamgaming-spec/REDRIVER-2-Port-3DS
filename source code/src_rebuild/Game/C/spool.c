...
#ifndef REDRIVER2_3DS_SPOOL_BOUNDARY
#define REDRIVER2_3DS_SPOOL_BOUNDARY 9
#endif
...
int check_regions_present(void)
{
...
#ifdef __3DS__
	near_boundary = REDRIVER2_3DS_SPOOL_BOUNDARY;
#else
	near_boundary = 9;
#endif
	far_boundary = MAP_REGION_SIZE - near_boundary;

	if (current_barrel_region_xcell < near_boundary)
	{
	...
	else if (current_barrel_region_xcell > far_boundary)
	{
		regions_to_unpack[0].xoffset = 1;
	...
	if (current_barrel_region_zcell < near_boundary && region_z != 0)
	{
		topbottom_unpack = 1;
	...
	else if(current_barrel_region_zcell > far_boundary && region_z != 0)
	{
		topbottom_unpack = 2;
...
extern char g_CurrentLevelFileName[MAX_GAME_PATH];
...
#define SPL_READ(dest, nsectors) \
		{ \
			int readSectors = (nsectors); \
			if (readSectors > 0) \
			{ \
				loadsectors((char*)(dest), spoolSector, readSectors); \
				spoolSector += readSectors; \
			} \
		}
#else
#define SPL_READ(dest, nsectors) \
