...
#define R2_3DS_TRACE(...) R2_3DS_Trace(__VA_ARGS__)
#else
#define R2_3DS_TRACE(...)
#endif

#ifndef REDRIVER2_3DS_QUICK_DRIVE
#define REDRIVER2_3DS_QUICK_DRIVE 0
#endif

#ifndef REDRIVER2_3DS_SKIP_INTRO
#define REDRIVER2_3DS_SKIP_INTRO 0
#endif
...
void State_GameInit(void* param)
{
	...
	R2_3DS_TRACE("State_GameInit: enter NewLevel=%d mission=%d GameType=%d", NewLevel, gCurrentMissionNumber, GameType);
	...
	R2_3DS_TRACE("State_GameInit: init variables begin");
	InitGameVariables();
	InitEvents();
	InitPadRecording();
	InitSpeechQueue(&gSpeechQueue);
	R2_3DS_TRACE("State_GameInit: init variables done");
	...
	R2_3DS_TRACE("State_GameInit: LoadMission %d begin", gCurrentMissionNumber);
	LoadMission(gCurrentMissionNumber);
	R2_3DS_TRACE("State_GameInit: LoadMission done GameLevel=%d", GameLevel);
#ifdef __3DS__
	if (maxCivCars > 2)
		maxCivCars = 2;
	if (maxCopCars > 1)
	...
	gEnableDlights = 0;
	R2_3DS_TRACE("State_GameInit: 3DS perf caps civ=%d cops=%d draw=%d", maxCivCars, maxCopCars, gDrawDistance);
#endif
	...
	if (NewLevel)
	{
		R2_3DS_TRACE("State_GameInit: ShowLoadingScreen begin");
		ShowLoadingScreen(LoadingScreenNames[GameLevel], 1, 36);
		R2_3DS_TRACE("State_GameInit: ShowLoadingScreen done");
	}
	...
	{
		ShowLoading();
		R2_3DS_TRACE("State_GameInit: LoadLevelSFX begin");
		LoadLevelSFX(gCurrentMissionNumber);
		R2_3DS_TRACE("State_GameInit: LoadLevelSFX done");
	}
	...
	{
		R2_3DS_TRACE("State_GameInit: LoadGameLevel begin");
		LoadGameLevel();
		R2_3DS_TRACE("State_GameInit: LoadGameLevel done");
		IconsLoaded = 1;
		R2_3DS_TRACE("State_GameInit: LoadSky begin");
		LoadSky();
		R2_3DS_TRACE("State_GameInit: LoadSky done");
		LoadFont(NULL);
	}
	...
	R2_3DS_TRACE("State_GameInit: spooling init begin");
	InitSpooling();
	InitMap();
	InitSpecSpool();
	R2_3DS_TRACE("State_GameInit: spooling init done");
	...
	int_garage_door();
	R2_3DS_TRACE("State_GameInit: SpoolSYNC begin");
	SpoolSYNC();
	R2_3DS_TRACE("State_GameInit: SpoolSYNC done");
	...
	R2_3DS_TRACE("State_GameInit: final game setup begin");
	StartGameSounds();
	...
	R2_3DS_TRACE("State_GameInit: final game setup done");
...
#ifdef __3DS__
	// The 3DS backend does not have an external event pump advancing the
	// PsyX VBlank counter during gameplay, so tick it here or the game
	// renders one frame and then waits forever before SetDispMask(1).
	VSync(0);
	frame++;
...
void State_GameLoop(void* param)
{
	int cnt;
#ifdef __3DS__
	static int firstLoopTrace = 1;

	if (firstLoopTrace)
	{
		R2_3DS_TRACE("State_GameLoop: enter FrameCnt=%d", FrameCnt);
		firstLoopTrace = 0;
	}
#endif
...
#ifdef __3DS__
	if (FrameCnt == 6)
		R2_3DS_TRACE("DrawGame: display enabled and frames advancing");
#endif
}
...
#endif // PSX
{
	...
	R2_3DS_TRACE("redriver2_main: enter argc=%d", argc);
	...
	R2_3DS_TRACE("redriver2_main: core init begin");
	SetDispMask(0);
	StopCallback();
	ResetCallback();
	ResetGraph(0);
	...
	Init_FileSystem();
	InitSound();
	R2_3DS_TRACE("redriver2_main: core init done");
	...
	R2_3DS_TRACE("redriver2_main: InitStringMng begin");
	if (!InitStringMng())
		R2_3DS_TRACE("redriver2_main: InitStringMng failed");
	else
		R2_3DS_TRACE("redriver2_main: InitStringMng done");
	
#endif
	{
		//PlayFMV(99);	// [A] don't show publisher logo

#if defined(__3DS__) && REDRIVER2_3DS_SKIP_INTRO
		R2_3DS_TRACE("redriver2_main: intro skipped for 3DS");
#else
		R2_3DS_TRACE("redriver2_main: ShowHiresScreens begin");
		ShowHiresScreens(ScreenNames, 300, 0); // [A]
		R2_3DS_TRACE("redriver2_main: ShowHiresScreens done");
		R2_3DS_TRACE("redriver2_main: PlayFMV begin");
		PlayFMV(0);		// play intro movie
		R2_3DS_TRACE("redriver2_main: PlayFMV done");
#endif
	}

	R2_3DS_TRACE("redriver2_main: CheckForCorrectDisc begin");
	CheckForCorrectDisc(0);
	R2_3DS_TRACE("redriver2_main: CheckForCorrectDisc done");
	
	// Init frontend
	R2_3DS_TRACE("redriver2_main: frontend overlay load begin");
	LOAD_OVERLAY("FRONTEND.BIN", _overlay_buffer);
	R2_3DS_TRACE("redriver2_main: frontend overlay load done");

	SpuSetMute(0);

	// initializes sound system
	R2_3DS_TRACE("redriver2_main: LoadSoundBankDynamic begin");
	LoadSoundBankDynamic(NULL, 0, 0);
	R2_3DS_TRACE("redriver2_main: LoadSoundBankDynamic done");

	R2_3DS_TRACE("redriver2_main: InitialiseScoreTables begin");
	InitialiseScoreTables();
	R2_3DS_TRACE("redriver2_main: InitialiseScoreTables done");

	// by default go to frontend
	R2_3DS_TRACE("redriver2_main: SetState init frontend");
	SetState(STATE_INITFRONTEND, (void*)2);

	R2_3DS_TRACE("redriver2_main: LoadCurrentProfile begin");
	LoadCurrentProfile(1);
	R2_3DS_TRACE("redriver2_main: LoadCurrentProfile done");

#if defined(__3DS__) && REDRIVER2_3DS_QUICK_DRIVE
	R2_3DS_TRACE("redriver2_main: 3DS quick drive setup begin");
	SetFEDrawMode();
	gInFrontend = 0;
	AttractMode = 0;
	NumPlayers = 1;
	...
	R2_3DS_TRACE("redriver2_main: 3DS quick drive setup done mission=%d", gCurrentMissionNumber);
#endif
	...
	R2_3DS_TRACE("redriver2_main: DoStateLoop begin");
	DoStateLoop();
	R2_3DS_TRACE("redriver2_main: DoStateLoop done");
